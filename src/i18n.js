...
export const MESSAGES = {
  en: {
    ...
    "init.created.body":
      ...
      "On its first reply, your AI is instructed to offer to scan your recent work and show you what changes (whether it actually does depends on your tool loading this rule) — you do not have to run anything for that. Want a read-only baseline yourself right now? Run: ai-collab bootstrap --yes (a local \"AI collaboration baseline\" report — nothing is written or sent anywhere).\n" +
...
Run now:
  ai-collab init --dry-run                  # preview only, writes nothing
  ai-collab init --target ./my-ai-workspace # write the workspace
...
`
  ...
  zh: {
    ...
    "init.created.body":
      ...
      "第一次回复时，你的 AI 被指示主动提出扫描你最近的工作、并演示它带来什么改变（是否真这样做取决于你的工具有没有加载这条规则）——这一步你不用自己运行任何东西。想现在自己就要一份只读基线？运行：ai-collab bootstrap --yes（本地「AI 协作基线」报告——不写入也不发送任何东西到任何地方）。\n" +
...
立即运行：
  ai-collab init --dry-run                  # 仅预览，不写入任何东西
  ai-collab init --target ./my-ai-workspace # 写出工作区
